 ...
 *
 * Patch note 2025-04-07 (Alexander C. Hubmann-Haidvogel <ahubmann@gmail.com>):
 * Minimal fix to restore functionality in Thunderbird 137, which dropped
 * XPCOMUtils.defineLazyGetter in favor of ChromeUtils.defineLazyGetter.
 *
 * Patch note 2026-09-05 (Alexander C. Hubmann-Haidvogel <ahubmann@gmail.com>):
 * - Remove WindowListener_102, only support Thunderbird 140+.
 * - Replace loadSubScript() with loadSubScriptWithOptions() and set
 *   allowUnsafeURL: true, otherwise "jar:" URLs are rejected,
 *   which are currently still needed for ThreadVis to work.
 *
...
const loadSubScript = (url, target) => Services.scriptloader.loadSubScriptWithOptions(url, { target, allowUnsafeURL: true });
