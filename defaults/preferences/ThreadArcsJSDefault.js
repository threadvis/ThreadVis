/* *******************************************************
 * ThreadArcsJSDefault.js
 *
 * (c) 2005-2006 Alexander C. Hubmann
 *
 * default preferences
 *
 * Version: $Id$
 ********************************************************/

pref("extensions.threadarcsjs.enabled", true);
pref("extensions.threadarcsjs.disabledaccounts", "");
pref("extensions.threadarcsjs.disabledfolders", "");
pref("extensions.threadarcsjs.logging.debug", false);
pref("extensions.threadarcsjs.logging.enabled", false);
pref("extensions.threadarcsjs.timescaling.enabled", true);
pref("extensions.threadarcsjs.timeline.enabled", true);
pref("extensions.threadarcsjs.visualisation.size", "12x12,12,12,32,6,2,24");
pref("extensions.threadarcsjs.visualisation.colour", "author");
pref("extensions.threadarcsjs.visualisation.highlight", true);
pref("extensions.threadarcsjs.defaultzoom.height", 1);
pref("extensions.threadarcsjs.defaultzoom.width", 1);
